...
import { i18nState, i18n, t } from './i18n.js'
...
<template>
  <div>
    <nav class="navbar">
      <div class="navbar-container">
        <router-link to="/" class="navbar-brand">
          <span>🎧</span>
          <span>{{ locale === 'ar' ? 'نظام الدعم' : 'SUPPORT CORE' }}</span>
        </router-link>
        ...
        <div class="nav-actions">
          ...
          <div v-else class="nav-actions">
            <router-link to="/login" class="nav-item" style="color: #fff;">{{ locale === 'ar' ? 'تسجيل الدخول' : 'Login' }}</router-link>
            <router-link to="/register" class="btn btn-primary btn-sm" style="border-radius: 20px;">
              {{ locale === 'ar' ? 'البدء الآن' : 'Get Started' }}
            </router-link>
          </div>
        </div>
      </div>
    ...
    <footer class="footer">
      <div class="container">
        <span>© 2026 {{ locale === 'ar' ? 'نظام تذاكر الدعم الفني' : 'Support Ticket System' }}</span>
      </div>
    </footer>
  </div>
</template>
